function dx = event_triggered_linear_rhs(x, x_hat, agent, graph, K_consensus)
% compute multi-agent linear dynamics using sampled states
% with event-triggered leader-following consensus
...
% distributed consensus control using sampled states
u = compute_event_triggered_control( ...
    x_hat, graph, K_consensus, agent.n, agent.m_input);


%------- leader directly connected to the desired reference
...
if ~isscalar(leader) || leader ~= round(leader) || ...
        leader < 1 || leader > N
    error('event_triggered_linear_rhs:BadLeader', ...
...
if numel(vector) ~= n
    error('event_triggered_linear_rhs:BadStateVector', ...
...
if numel(vector) ~= m_input
    error('event_triggered_linear_rhs:BadInputVector', ...
...
if ~isequal(size(K_reference), [m_input, n])
    error('event_triggered_linear_rhs:BadReferenceGain', ...
