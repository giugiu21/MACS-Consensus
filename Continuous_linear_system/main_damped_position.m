%DAMPED SYSTEM - -> posizione desiderata caso leader -> caso continuo |->caso senza leader
...
p_desired = 2; %position
v_desired = 0; %velocity
...
fprintf('Leader ID: %d\n', leader_id);
fprintf('Desired position: %.4f\n', p_desired);
fprintf('Equilibrium feedforward: %.4f\n', u_desired);
...
plot( ...
    time(plot_mask), ...
    desired_position_history(plot_mask), ...
    'k--', ...
    'LineWidth', 1.5);
...
animate_mass_consensus( ...
    time, ...
    r_trig_free.y_history, ...
    agent, ...
    N, ...
    fullfile( ...
        results_dir, ...
        'leaderless_trigger_consensus.mp4'));

animate_mass_consensus( ...
    time, ...
    r_trig_leader.y_history, ...
    agent, ...
    N, ...
    fullfile( ...
        results_dir, ...
        'leader-follower_trigger_consensus.mp4'));
